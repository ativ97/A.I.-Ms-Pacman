        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for p in newPosDist.keys():
                allPossible[p] += oldBelief * newPosDist[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while n > 0:
            if n < len(self.legalPositions):
                uI += self.legalPositions[0:n - 1]
                n=0
            else:
                uI += self.legalPositions
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            myParticles.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))))
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                jailIndices.append(i)

        Count = util.Counter()

        for particle in self.particles:
            currentProbability = 1
            for i in jailIndices:
                particle = self.getParticleWithGhostInJail(particle, i)
            for i in range(self.numGhosts):
                if i not in jailIndices:
                    currentProbability *= emissionModels[i][util.manhattanDistance(particle[i], pacmanPosition)]
            Count[particle] += currentProbability
        self.beliefs = Count
