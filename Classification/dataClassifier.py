...
def enhancedFeatureExtractorDigit(datum):
    ...
    for y in range(DIGIT_DATUM_HEIGHT):
        ...
        prev = 0
        for x in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x, y) != prev:
                count += 1
                prev = datum.getPixel(x, y)
        if count > 1:
...
def enhancedPacmanFeatures(state, action):
    ...
    features['nearestghostdistance'] = min(util.manhattanDistance(l, successor.getPacmanPosition()) for l in successor.getGhostPositions())
    return features
